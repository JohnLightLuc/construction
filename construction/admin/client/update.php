<?php 
  ...
  $array = array('id'=>'','nom'=>'','prenom'=>'','contact'=>'', 'email'=>'','ville'=>'','password'=>'','password1'=>'','password2'=>'','photo'=>'','imageError'=>'','error'=>'', 'message'=>'', 'verify'=>false,'success'=>false);
  $array['verify'] = false;
    ...
    if($array['verify'] )
    {
        ...
        else
        {
                ...
                $db = Database::connect();
                    ...
                    if(empty($array['photo'])){
                        $statement = $db->prepare("UPDATE `clients` SET `nom`=?,`prenom`=?,`contact`=?,`email`=?,`ville`=?,`password`=?,`photo`=? WHERE id=?");
                        $statement->execute(array($array['nom'], $array['prenom'],$array['contact'],$array['email'], $array['ville'], $array['password1'], $array['id']));
                    ...
                    else
                    {
                        ...
                        $statement = $db->prepare("UPDATE `clients` SET `nom`=?,`prenom`=?,`contact`=?,`email`=?,`ville`=?,`password`=?,`photo`=? WHERE id=?");
                        $statement->execute(array($array['nom'], $array['prenom'],$array['contact'],$array['email'], $array['ville'], $array['password1'],$array['photo'], $array['id']));
                        header("Location: ../accueil.php");
                    }
...
{ 
    ...
    $db = Database::connect();
    $requete = $db->prepare("SELECT * FROM clients WHERE id = ?");
        ...
        <div class="inscrit_client">
         ...
         <h1>MODIFIER UN CLIENT</h1>
          ...
          <div class="alert">
            ...
            <div class="form-group row">
              <div class="col-sm-10">
                <input type="text" name="prenom"  class="form-control" id="prenom" placeholder="Nom" value="<?php echo $array['prenom']; ?>">
              </div>
            </div>
          ...
          <div class="form-group row">
              <div class="col-sm-10">
                <input type="password" name="password1"  class="form-control" id="password1" placeholder="Mot de passe" value="<?php echo $array['password']; ?>" >
              </div>
          </div>

          <div class="form-group row">
              <div class="col-sm-10">
                <input type="password" name="password2"  class="form-control" id="password2"  placeholder="Entrer à nouveau le mot de passe"  value="<?php echo $array['password']; ?>">
              </div>
          </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                ...
                <small> <?php echo $array['imageError']; ?></small>
              </div>
            </div>
