...
<?php 
  ...
  $array = array('id'=>'','client'=>'','fournisseur'=>'','materiel'=>'', 'quantite'=>'','date'=>'','error'=>'', 'message'=>'', 'verify'=>false,'success'=>false);
  $array['verify'] = false;
...
{
    $db = Database::connect();
    $statement = $db->prepare("SELECT * FROM `commande` WHERE id = ?");
    $statement->execute(array($array['id']));
    $row = $statement->fetch();
    
    $array['client'] = $row['client'];
    $array['fourniseur'] = $row['fournisseur'];
    $array['materiel'] = $row['materiel'];
    $array['quantite'] = $row['quantite'];
         ...
         <h1>Commader votre matériel</h1>
          ...
          <div class="form-group row">
                ...
                <select class="form-control" name="materiel" id="article">
                  ...
                  <?php
                      ...
                      while($row = $statement->fetch())
                      {
                          if($row['id'] == $array['id'])
                          {
                            echo "<option selected value='".$row['id']."'>".$row['libelle']."</option>";
                          }else{
            ...
            <label for="article" class="col-sm-2 col-form-label">Quantité:</label>
              <div class="col-sm-8">
                <input type="text" name="quantite"  class="form-control" id="quantite" placeholder="Quantité de materiel souhaité" value="<?php echo $array['quantite']; ?>">
              </div>
            </div>
